...
--
-- 2026-09-13 NOTE: the settle_in_process_* -> ready_to_settle_* renames that
-- were here are DROPPED (never ran live). Live DB + app
-- (SupabaseClaimsReader/Writer) + claims Edge Function all use
-- settle_in_process_by_uid/system_id/at — running those renames would break
-- reads/writes. settle_in_process_* is the final name; the function block
-- below already uses it (matches 202609050004 + live pg_get_functiondef).
